...
--4 Napisz polecenie, które wybiera numer tytułu i tytuł dla wszystkich książek, których tytuły zawierających słowo „adventure”
SELECT title_no,title FROM title WHERE title LIKE '%adventure%'

--5 Napisz polecenie, które wybiera numer czytelnika, oraz zapłaconą karę
SELECT member_no, isnull(fine_paid,0) AS finePaid 
FROM loanhist WHERE isnull(fine_paid,0)=0 and isnull(fine_assessed,0)>0

--6 Napisz polecenie, które wybiera wszystkie unikalne pary miast i stanów z tablicy adult.
SELECT DISTINCT city,state FROM adult

--7  Napisz polecenie, które wybiera wszystkie tytuły z tablicy title i wyświetla je w porządku alfabetycznym.
SELECT title FROM title ORDER BY title 

/*
8. Napisz polecenie, które:
...
/*
10. Napisz polecenie, które wybiera title i title_no z tablicy title.
